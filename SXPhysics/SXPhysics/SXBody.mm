...
#pragma mark - Fixtures add/remove

-(void)addFixture:(b2FixtureDef *)fixtureDef withName:(NSString *)fixtureName
{
    [shapeDataList setObject:[NSValue valueWithPointer:fixtureDef] forKey:fixtureName];
}
...
#pragma mark - Add/remove child with fixture
...
-(void) addChild:(SPDisplayObject *)child withFixture:(b2FixtureDef *)fixtureDef withName:(NSString *)fixtureName
{
    ...
    [self addChild:child];
    [self addFixture:fixtureDef withName:fixtureName];
}
...
+(SXBody*)createBodyForBodyName:(NSString*)name fromFile:(NSString*)plist
{
    ...
    float customPTMRatio =  [[metadataDict objectForKey:@"ptm_ratio"] floatValue];
    ...
    // Create the body
    SXBody *newBody = [SXBody body];
    
    // Search in bodies list
    for(NSString *bodyName in bodyDict)
    {
        if ([bodyName isEqualToString:name])
        {
            ...
            // TODO: Figure out how to use this anchor point
            CGPoint anchorPoint = CGPointFromString_([bodyData objectForKey:@"anchorpoint"]);
            ...
            for(NSDictionary *fixtureData in fixtureList)
            {
                ...
                if([fixtureType isEqual:@"POLYGON"])
                {
                    ...
                    for(NSArray *polygonArray in polygonsArray)
                    {
                        ...
                        fixture->filter.categoryBits = [[fixtureData objectForKey:@"filter_categoryBits"] intValue];
                        fixture->filter.maskBits = [[fixtureData objectForKey:@"filter_maskBits"] intValue];
                        fixture->filter.groupIndex = [[fixtureData objectForKey:@"filter_groupIndex"] intValue];
                        fixture->friction = [[fixtureData objectForKey:@"friction"] floatValue];
                        fixture->density = [[fixtureData objectForKey:@"density"] floatValue];
                        fixture->restitution = [[fixtureData objectForKey:@"restitution"] floatValue];
                        fixture->isSensor = [[fixtureData objectForKey:@"isSensor"] boolValue];
                        ...
                        [newBody addFixture:fixture withName:[NSString stringWithFormat:@"%@-polygon%02d", bodyName, [polygonsArray indexOfObject:polygonArray]]];
                    }
                }
                else if([fixtureType isEqual:@"CIRCLE"])
                {
                    ...
                    b2CircleShape *circleShape = new b2CircleShape();
                    circleShape->m_radius = [[circleData objectForKey:@"radius"] floatValue]  / customPTMRatio;
                    CGPoint p = CGPointFromString_([fixtureData objectForKey:@"center"]);
                    circleShape->m_p = b2Vec2(p.x / customPTMRatio, p.y / customPTMRatio);
                    fixture->shape = circleShape;
                    
                    [newBody addFixture:fixture withName:[NSString stringWithFormat:@"%@-circle", bodyName]];
                }
                else
                {
                    // unknown type
